...
def filter_attenuator_by_energy(attenuator_position, energy):
    # Remove foils from the list if the energy value
    # is not in their linear absorption range

    # Zr foils must only be used if energy >= 18.5 keV
    if energy >= 18.5:
        return attenuator_position
    attenuator_position = [item for item in attenuator_position if '_Zr' not in item]

    # Au foils must only be used if energy >= 15.0 keV
    if energy >= 15.0:
        return attenuator_position
    attenuator_position = [item for item in attenuator_position if '_Au' not in item]

    # Cu foils must only be used if energy >= 9.5 keV
    if energy >= 9.5:
        return attenuator_position
    attenuator_position = [item for item in attenuator_position if '_Cu' not in item]

    # Ti foils must only be used if energy >= 6.5 keV
    if energy >= 6.5:
        return attenuator_position
    attenuator_position = [item for item in attenuator_position if '_Ti' not in item]

    # If energy < 6.5 keV, use only Al foils
    print("Filtered: ", attenuator_position)
    return attenuator_position
...
def get_transmission(energy, transmission):
    ...
    attenuator_position = filter_attenuator_by_energy(attenuator_position, energy)
    ...
    print("filter_combination: ", filter_combination)
